...
#pragma warning restore IDE0044 // Add readonly modifier
        ...
        private readonly CookieCollection _rawCookies = new CookieCollection();
        ...
        public byte[] ToBytes()
        {
            ...
            catch (Exception ex)
            {
                ...
                {
                    throw NewHttpException("Could not load the message body of the response HTTP-server '{0}'.", ex);
                }
        ...
        internal long LoadResponse(HttpMethod method)
        {
            ...
            catch (Exception ex)
            {
                ...
                {
                    throw NewHttpException("Failed to load the response from the HTTP-server '{0}'.", ex);
                }
        ...
        private void ReceiveStartingLine()
        {
            ...
            while (true)
            {
                ...
                if (startingLine.Length == 0)
                {
                    HttpException exception =
                        NewHttpException("Received empty response from the HTTP-server '{0}'.");
            ...
            {
                throw NewHttpException("Received empty response from the HTTP-server '{0}'.");
            }
        ...
        private void SetCookie(string value)
        {
            ...
            if (separatorPos == -1)
            {
                string message = string.Format(
                    "Received invalid cookies '{0}' from the HTTP-server '{1}'.", value, Address.Host);
        ...
        private void ReceiveHeaders()
        {
            while (true)
            {
                ...
                {
                    string message = string.Format(
                        "Received invalid header \"{0} 'from the HTTP-server' {1} '.", header, Address.Host);
                ...
                catch (Exception ex)
                {
                    if (ex is FormatException || ex is OverflowException)
                    {
                        throw NewHttpException(string.Format(
                            "Received invalid data block size when using Chunked: {0}", line), ex);
                    }
        ...
        private void WaitData()
        {
            ...
            while (!_request.ClientNetworkStream.DataAvailable)
            {
                if (sleepTime >= delay)
                {
                    throw NewHttpException("It turned out the wait time data from the HTTP-server '{0}'.");
                }
            ...
            switch (contentEncoding)
            {
                ...
                default:
                    throw new InvalidOperationException(string.Format(
                        "Received an unsupported encoding format: {0}", contentEncoding));
            }
        }
