...
namespace xNet
{
    ...
    public class HttpProxyClient : ProxyClient
    {
            ...
            if (destinationPort != 80)
            {
                ...
                if (statusCode != HttpStatusCode.OK)
                {
                    ...
                    throw new ProxyException(string.Format(
                        "Received invalid status code '{0}' on '{1}' proxy.", statusCode, ToString()), this);
                }
            }
        ...
        private HttpStatusCode ReceiveResponse(NetworkStream nStream)
        {
            ...
            {
                throw NewProxyException("Received empty response from the proxy server '{0}'.");
            }
            ...
            if (simPos == -1)
            {
                throw NewProxyException("Received an invalid response from the proxy server '{0}'.");
            }
        ...
        private void WaitData(NetworkStream nStream)
        {
            ...
            while (!nStream.DataAvailable)
            {
                if (sleepTime >= delay)
                {
                    throw NewProxyException("It turned out the wait time data from the proxy server '{0}'.");
                }
