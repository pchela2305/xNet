				...
				catch (Exception ex)
				{
					if (ex is SocketException || ex is ArgumentException)
					{
						throw new ProxyException(string.Format(
							"Failed to get the host IP address '{0}'.", destinationHost), this, ex);
					}
        ...
        internal protected void HandleCommandError(byte command)
        {
            ...
            switch (command)
            {
                case CommandReplyRequestRejectedOrFailed:
                    errorMessage = "Query rejected or erroneous.";
                    break;

                case CommandReplyRequestRejectedCannotConnectToIdentd:
                    errorMessage = "The request failed, because things are not running identd (or not available from the server).";
                    break;

                case CommandReplyRequestRejectedDifferentIdentd:
                    errorMessage = "The request failed because client's identd could not confirm the user ID in the query.";
                    break;
