...
namespace xNet
{
    ...
    public class Socks5ProxyClient : ProxyClient
    {
        ...
        private void SendUsernameAndPassword(NetworkStream nStream)
        {
            ...
            {
                throw NewProxyException("Failed to authenticate with the proxy server '{0}'.");
            }
        }
        ...
        private byte GetAddressType(string host)
        {
			...
			switch (ipAddr.AddressFamily)
            {
                ...
                default:
                    throw new ProxyException(string.Format("The host '{0}' type '{1}' does not support the type Adresse. The following types: InterNetwork and InterNetworkV6. The proxy server '{2}'.",
        ...
        private void HandleCommandError(byte command)
        {
            ...
            switch (command)
            {
                case AuthMethodReplyNoAcceptableMethods:
                    errorMessage = "The proposed authentication methods are supported.";
                    break;

                case CommandReplyGeneralSocksServerFailure:
                    errorMessage = "Error SOCKS-server.";
                    break;

                case CommandReplyConnectionNotAllowedByRuleset:
                    errorMessage = "Connecting a set of rules is prohibited.";
                    break;
                ...
                case CommandReplyTTLExpired:
                    errorMessage = "Expired TTL.";
                    break;

                case CommandReplyCommandNotSupported:
                    errorMessage = "The command is not supported or protocol error.";
                    break;
            ...
            string exceptionMsg = string.Format(
                "{0} The proxy server '{1}'.", errorMessage, ToString());
